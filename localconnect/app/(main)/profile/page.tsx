...
export default function ProfilePage() {
  ...
  async function handleEnableNotifications() {
    ...
    setNotifStatus(
      result === "subscribed"
        ...
        : result === "denied"
        ? "You blocked notifications — enable them in your browser's site settings to turn this on."
        : result === "not-configured"
        ? "Push isn't set up yet — see supabase/PUSH_SETUP.md"
  ...
  async function handlePhotoUpload(file: File) {
    ...
    const url = await uploadToBucket(supabase, "avatars", profile.id, file, "avatar");
    if (url) {
    ...
    <div className="px-[18px] py-4">
      <div className="flex items-center gap-3 mb-2">
        <div className="aurora-ring flex-shrink-0 relative">
          <Avatar url={profile.avatar_url} initials={profile.initials} bg={profile.avatar_bg} fg={profile.avatar_fg} size={56} />
          <label className="absolute -bottom-1 -right-1 w-6 h-6 bg-aurora rounded-full flex items-center justify-center cursor-pointer border-2 border-bg">
            <span className="text-[11px]">{uploadingPhoto ? "…" : "📷"}</span>
            <input
              type="file"
              accept="image/*"
              className="hidden"
      ...
      <div className="flex flex-col gap-3">
        ...
        {notifStatus && <p className="text-xs text-muted text-center -mt-1.5">{notifStatus}</p>}

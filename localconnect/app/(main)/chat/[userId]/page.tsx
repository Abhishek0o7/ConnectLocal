      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 flex flex-col gap-2.5">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex flex-col ${m.sender_id === me ? "items-end" : "items-start"}`}>
            <div
              className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed pop-in relative ${
                m.sender_id === me
                  ? "bg-aurora text-white rounded-br-md"
                  : "glass bg-surface/60 text-ink rounded-bl-md border border-hairline"
              }`}
            >
              {m.photo_url ? (
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                {m.sender_id === me && (
                  <button
                    ...
                    className="text-[10px] opacity-60 bg-transparent border-none ml-auto"
            ...
            {confirmDeleteId === m.id && (
              <div className="flex items-center gap-2 mt-1 pop-in">
                <span className="text-[11px] text-muted">Delete this {m.photo_url ? "photo" : m.audio_url ? "voice note" : "message"}?</span>
                <button
